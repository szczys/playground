...
class Grid:
    ...
    def gather_gears(self):
        gear_dict = dict()
        for s in self.syms:
            if s.value == '*':
                gear_dict[(s.x, s.y)] = list()

        for n in g.nums:
            r = g.get_ranges(n)
            for s in gear_dict:
                if s[0] in r.xrange and s[1] in r.yrange:
                    gear_dict[s].append(int(n.value))

        sum = 0
        for s in gear_dict:
            if len(gear_dict[s]) == 2:
                sum += (gear_dict[s][0] * gear_dict[s][1])
